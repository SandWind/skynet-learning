...
local function unpack_line(text)
   local form=text:find("\n",1,true)
   if form then
      return text:sub(1,form-1),text:sub(form+1)
   end
   return nil,text
end
...
local function encode_token(token)
   ...
   crypt.base64encode(token.server),crypt.base64encode(token.pass))
end
...
local subid=crypt.base64encode(string.sub(result,5))
...
print(readpackage())
print("===>",send_request("fake",0))
print("===>",send_request("again",1))
